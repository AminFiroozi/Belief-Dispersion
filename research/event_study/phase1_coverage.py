...
def previous_trading_dates(
    ...
    return set(previous.date).union({event_date.date()})
...
def audit_coverage(
    ...
    for event in events.itertuples(index=False):
        ...
        for window in WINDOWS:
            allowed_dates = previous_trading_dates(
                trading_dates, event.release_timestamp_et, window
            )
            selected = candidates.loc[
                candidates["local_date"].isin(allowed_dates)
            ...
            rows.append(
                {
                    ...
                    "window_definition": f"{window} completed sessions plus event-day pre-release news",
